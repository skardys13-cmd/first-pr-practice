...
(async()=>{
 ...
 t('task 1 is a list task',(await p.$('[data-add]'))!==null);
 ...
 t('items persist to storage',await p.evaluate(i=>{const s=JSON.parse(localStorage.getItem('opsladder.v1'));return s.work[i].items.length===3;},id));
 ...
 // survives reload, still open? (openWs is in-memory by design) -> content survives
 ...
 await p.click('#todayCard .workbtn[data-ws]'); await p.waitForTimeout(300);
 t('work survives reload',(await p.$$('.item')).length===3);
